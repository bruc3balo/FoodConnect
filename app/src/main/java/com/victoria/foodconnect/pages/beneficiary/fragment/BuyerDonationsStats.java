...
import com.victoria.foodconnect.databinding.ActivityBuyerDonationsStatsBinding;
...
public class BuyerDonationsStats extends AppCompatActivity {

    private ActivityBuyerDonationsStatsBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityBuyerDonationsStatsBinding.inflate(getLayoutInflater());
        ...
        userRepository.getUserLive().observe(this, appUser -> appUser.ifPresent(u->{
            ...
            binding.toolbar.setSubtitle(u.getUsername()+", donation stats");
        }));
    ...
    private void setUpBuyerDonationsBarChart(List<Models.SellerStats> allDonorStats, MyLinkedMap<String, String> donors) {
        ...
        binding.title.setText("Donations received for "+username+" in 2022");

        cartesian.yScale().minimum(0d);
        cartesian.xAxis(0).title("Months").labels().rotation(90).padding(0d, 0d, 20d, 0d);
        cartesian.yAxis(0).title("Number of complete purchases").labels().format("{%Value}{groupsSeparator: }").rotation(45);
        ...
        cartesian.tooltip().positionMode(TooltipPositionMode.POINT).position("right").anchor(Anchor.LEFT_CENTER).offsetX(5d).offsetY(0d).format("{%Value} donations received");
    ...
    private void populateBeneficiaryChart(String username) {
        ...
        new ViewModelProvider(this).get(StatsViewModel.class).getBeneficiaryDonationsStatsLive(2022,username).observe(this, sellerStats -> {

            if (sellerStats.isEmpty() || sellerStats.get(0).getProducts().isEmpty()) {
                Toast.makeText(BuyerDonationsStats.this, "No data", Toast.LENGTH_SHORT).show();
                outSpinnerProgress(binding.donorPb, null);
            ...
            setUpBuyerDonationsBarChart(allDonorStats, donors);
