...
import com.victoria.foodconnect.databinding.ActivityTransporterDonationStatsBinding;
...
public class TransporterDonationStats extends AppCompatActivity {

    private ActivityTransporterDonationStatsBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityTransporterDonationStatsBinding.inflate(getLayoutInflater());
        ...
        userRepository.getUserLive().observe(this, appUser -> appUser.ifPresent(u->{
            ...
            populateTransporterDonationsChart(u.getUsername());
            binding.toolbar.setSubtitle(u.getUsername()+", donations stats");
        }));
        setWindowColors(this);

    }

    private void setUpTransporterCompleteDonationsBarChart(List<Models.SellerStats> allDonorStats, MyLinkedMap<String, String> donors) {
        ...
        binding.title.setText("Donations, of "+username+", transported in 2022");

        cartesian.yScale().minimum(0d);
        cartesian.xAxis(0).title("Months").labels().rotation(90).padding(0d, 0d, 20d, 0d);
        cartesian.yAxis(0).title("Number of complete donations").labels().format("{%Value}{groupsSeparator: }").rotation(45);
        ...
        cartesian.tooltip().positionMode(TooltipPositionMode.POINT).position("right").anchor(Anchor.LEFT_CENTER).offsetX(5d).offsetY(0d).format("{%Value} donations delivered");
    ...
    private void populateTransporterDonationsChart(String username) {
        ...
        new ViewModelProvider(this).get(StatsViewModel.class).getTransporterDonationsStatsLive(2022,username).observe(this, sellerStats -> {

            if (sellerStats.isEmpty() || sellerStats.get(0).getProducts().isEmpty()) {
                Toast.makeText(TransporterDonationStats.this, "No data", Toast.LENGTH_SHORT).show();
                outSpinnerProgress(binding.donorPb, null);
            ...
            setUpTransporterCompleteDonationsBarChart(allDonorStats, donors);
